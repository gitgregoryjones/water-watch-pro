...
const PRICE_BY_PLAN: Record<string,string> = {
  platinum: process.env[`UPGRADE_2027`] && process.env[`UPGRADE_PRICE_ID_PLATINUM`] || process.env[`PRICE_ID_PLATINUM`]!,
  gold: process.env[`UPGRADE_2027`] && process.env[`UPGRADE_PRICE_ID_GOLD`] || process.env[`PRICE_ID_GOLD`]!,
  silver: process.env[`UPGRADE_2027`] && process.env[`UPGRADE_PRICE_ID_SILVER`] || process.env[`PRICE_ID_SILVER`]!,
  bronze: process.env[`UPGRADE_2027`] && process.env[`UPGRADE_PRICE_ID_BRONZE`] || process.env[`PRICE_ID_BRONZE`]!,
  trial: process.env[`UPGRADE_2027`] && process.env[`UPGRADE_PRICE_ID_TRIAL`] || process.env[`PRICE_ID_TRIAL`]!, // if you use it
};
