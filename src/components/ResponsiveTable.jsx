...
function ResponsiveTable({location = {}}) {
  ...
  const threshold = location.atlas14_threshold ? location.atlas14_threshold : []
        ...
        <tbody className="overflow-y-scroll h-[5rem] max-h-[5rem]">
          {data.map((row, index) => (
            ...
            <tr key={index} className="odd:bg-white even:bg-gray-50">
              ...
              <td className="border px-4 py-2">{threshold['24h'] ? threshold['24h'][index]?.toFixed(2) : "No Data"}</td>
              <td className="border px-4 py-2">{threshold['1h'] ? threshold['1h'][index]?.toFixed(2) : "No Data"}</td>
            </tr>
          ))}
        </tbody>
      </table>
