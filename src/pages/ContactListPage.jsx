...
const ContactListPage = () => {
  ...
  const fetchContacts = async (page) => {
    try {
      ...
      if (user.role == "admin") {
        url = isActive("unarchive-contacts") 
          ? `/api/contacts/all/?a=true&status=all` 
          : `/api/contacts/all/?a=true`;
      }
  ...
  const filteredContacts = useMemo(() => {
    ...
    return contacts.filter(contact => {
      ...
      const phoneMatch = contact.phone?.toString().toLowerCase().includes(searchLower) || false;
  ...
  return (
    ...
    >               <td className="text-sm border border-gray-300 p-2 text-center">{contact.id}</td>
              ...
              <tbody className='relative'>
                <tr>
                  <td colSpan="7" className='flex items-center justify-center font-bold p-4'>
